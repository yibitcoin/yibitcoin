...
// YiBitcoin: Normally minimum difficulty blocks can only occur in between
...
unsigned int CalculateYiBitcoinNextWorkRequired(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params)
{
...
CAmount GetYiBitcoinBlockSubsidy(int nHeight, const Consensus::Params& consensusParams, uint256 prevHash)
{	
...
int64_t GetYiBitcoinDustFee(const std::vector<CTxOut> &vout, CFeeRate &baseFeeRate) {
    int64_t nFee = 0;
