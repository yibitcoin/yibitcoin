...
*Setup instructions for a gitian build of YiBitcoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the YiBitcoin
...
- [Building YiBitcoin](#building-yibitcoin)
...
Clone the git repositories for yibitcoin and gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/yibitcoin/yibitcoin
```
...
Follow the instructions in [doc/release-process.md](release-process.md) in the yibitcoin repository
...
To build YiBitcoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the yibitcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/yibitcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/yibitcoin/yibitcoin
...
For example:
```bash
URL=https://github.com/laanwj/yibitcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit yibitcoin=${COMMIT} --url yibitcoin=${URL} ../yibitcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit yibitcoin=${COMMIT} --url yibitcoin=${URL} ../yibitcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit yibitcoin=${COMMIT} --url yibitcoin=${URL} ../yibitcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/yibitcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/yibitcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/yibitcoin-osx-build.assert
```
...
[yibitcoin/gitian.sigs](https://github.com/yibitcoin/gitian.sigs/) repository, or if that's not possible create a pull request. 
