UNIX BUILD NOTES
====================
Some notes on how to build YiBitcoin in Unix. 

Note
---------------------
Always use absolute paths to configure and compile YiBitcoin and the dependencies,
...
This will build YiBitcoin-Qt as well if the dependencies are met.
...
memory available when compiling YiBitcoin Core. With 512MB of memory or less,
...
If you want to build YiBitcoin-Qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a yibitcoin-qt executable will be
built by default.

Notes
-----
The release is built with GCC and then "strip yibitcoind" to strip the debug
...
# Pick some path to install BDB to, here we create a directory within the yibitcoin directory
...
# Configure YiBitcoin Core to use our own-built instance of BDB
cd $BITCOIN_ROOT
...
Security
--------
To help make your YiBitcoin installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, YiBitcoin should be built with a non-executable stack,
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, YiBitcoin may be compiled in
